...
def list_doctors(specialization: Optional[str] = None, current_user: models.User = Depends(require_patient), db: Session = Depends(get_db)):
...
def book_appointment(req: schemas.AppointmentCreate, current_user: models.User = Depends(require_patient), db: Session = Depends(get_db)):
    ...
    db.add(notif)
    print(f"📱 SMS to {current_user.phone}: {notif_msg}")
