...
import models, schemas, gangadhar.auth as auth
...
def call_next(appointment_id: int, background_tasks: BackgroundTasks, current_user: models.User = Depends(require_doctor), db: Session = Depends(get_db)):
    ...
    if current_in_progress:
        ...
        notif_msg = f"Thank you for visiting Dr. {doctor.user.name} at SmartQueue AI! We are happy to serve you and wish you a speedy recovery! 🌟"
    ...
    notif_msg = f"🔔 The Doctor is ready for you! Please proceed to Dr. {doctor.user.name}'s room. Token #{next_appt.token_number}."
    if sid and token and phone:
        if not phone.startswith("+"):
            phone = "+91" + phone
        try:
